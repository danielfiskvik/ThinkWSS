...
namespace SimpleWeb {
    ...
    public:
        ...
        class Connection {
            // SocketServerBase og SocketServer har adgang til de private delene av Connection
       ...
       void start() {
        ...
        void send_close(std::shared_ptr<Connection> connection, int status, const std::string& reason = "") {
            ...
            send(connection, response, [](const boost::system::error_code & ec) {
            }, 136);
        }
    ...
    public:
        ...
        SocketServerBase<WS>::SocketServerBase(port, num_threads, timeout_request, timeout_idle) {
            //cout << port << ", " << num_threads << ", " << timeout_request << ", " << timeout_idle << endl;
        };
